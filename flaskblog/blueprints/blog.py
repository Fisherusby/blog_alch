...
from flaskblog.models import User, Blog, Category, Comment, Review
from flaskblog.forms import BlogForm, CommentForm, ReviewForm
from flaskblog.login import is_auth, is_anonim, is_admin
from sqlalchemy.sql import func, desc, label, or_
...
def detail(blog_id):
    blog = Blog.query.filter_by(id=blog_id).filter(or_(Blog.is_public, Blog.author == g.user)).first()
    ...
    db.session.commit()
    return render_template('blog/blog_detail.html', blog=blog, comment_form=comment_form)
...
@is_auth
def favorite_blogs():
    blogs_list = Blog.query.filter_by(is_public=True).filter(Blog.in_favorite.any(id=g.user.id)).all()
    return render_template('blog/blog_list.html', blogs_list=blogs_list)
