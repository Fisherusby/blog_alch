...
class Blog(db.Model):
    ...
    in_favorite = db.relationship('User', secondary=mtm_in_favorite, lazy='subquery',
                                  backref=db.backref('favorite_blogs', lazy=True)
                                  )
    # set relation for like with User
    likes = db.relationship('User', secondary=mtm_likes, lazy='subquery',
                            backref=db.backref('likes_blogs', lazy=True)
                            )
    # set relation for dislike with User
    dislikes = db.relationship('User', secondary=mtm_dislikes, lazy='subquery',
...
class Tag(db.Model):
    ...
    # set relation with Blog
    blogs = db.relationship('Blog', secondary=mtm_tags, lazy='subquery',
